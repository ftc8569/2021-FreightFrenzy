...
public class MiniPID {
    ...
    /**
     * SAME AS REGULAR GETOUPUT BUT MODIFIED TO CORRECTLY GET THE ERROR OF ANGLES
     * Calculate the output value for the current PID cycle.<br>
     * @param actual The monitored value, typically as a sensor input.
     * @param setpoint The target value for the system
     * @return calculated output value for driving the system
     */
    public double getOutputAngles(double actual, double setpoint){
        double output;
        double Poutput;
        double Ioutput;
        double Doutput;
        ...
        double error = Math.toDegrees(Math.atan2(Math.sin(x - y), Math.cos(x - y)));
        lastError = error;
     ...
     * @return calculated output value for driving the system
     */
    public double getOutputAngles(){
        return getOutputAngles(lastActual,setpoint);
    }
         ...
         * @param strength output valid between [0..1), meaning [current output only.. historical output only)
         */
